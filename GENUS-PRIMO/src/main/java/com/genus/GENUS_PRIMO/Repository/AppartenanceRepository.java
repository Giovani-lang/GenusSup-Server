...
public interface AppartenanceRepository extends JpaRepository<Appartenance,Long> {
    @Query(value = "SELECT a FROM Appartenance a WHERE a.option.classe.cycle.campus.ecole.id =:e AND a.anneeAcademique.id =:a")
    List<Appartenance> findByAnneeId (@Param("e") Long ecoleId,@Param("a") Long anneId);
    @Query(value = "SELECT a FROM Appartenance a WHERE a.option.classe.cycle.campus.ecole.id =:e")
